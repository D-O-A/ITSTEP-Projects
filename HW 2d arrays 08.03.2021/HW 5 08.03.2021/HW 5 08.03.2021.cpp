...
int main()
{
	...
	//Написать программу, которая определяет сумму значений, находящихся в массиве
	...
	if (minIndexRow > maxIndexRow || minIndexRow >= maxIndexRow && minIndexCol > maxIndexCol)   // меняем местами если минимальный элемент идет после максимального
	{
	...
	int n;
	if (minIndexCol == COLUMN - 1)   // делаем магию с правильным началом суммирования элементов
		minIndexRow += 1, n = 0;
	else
		n = minIndexCol + 1;
